...
def plot_skeleton_kpts(im, kpts, steps,depth):
    ...
    for kid in range(num_kpts):
        ...
        #print(x_coord,y_coord)#######坐标
        if not (x_coord % 640 == 0 or y_coord % 640 == 0):
            if steps == 3:
                conf = kpts[steps * kid + 2]#   看是否可见决定是否画点
                if conf < 0.5:
                    kpts_x_y_v_dep.append([int(x_coord), int(y_coord), conf, -1])###########
                    continue
            try:
                kpts_x_y_v_dep.append([int(x_coord), int(y_coord), conf, depth[int(x_coord), int(y_coord)]])
            except:
                kpts_x_y_v_dep.append([int(x_coord), int(y_coord), conf, -1])
    ...
    #print(kpts_x_y_v_dep)
    return im,kpts_x_y_v_dep
