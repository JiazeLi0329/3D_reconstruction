...
from utils_3d import create_segment, is_present, find_plane_points, find_plane, create_dynamic_grid, determine_color, coordinates_dynamic_grid, create_kpts_xyd
from sklearn.linear_model import RANSACRegressor

LINES_BODY = [[1, 2], [1, 3], [2, 3], [2, 4], [3, 5], [4, 6], [5, 7], [6, 7], [6, 8], [6, 12], [7, 9], [7, 13], [8, 10],
              [9, 11], [12, 13], [12, 14], [13, 15], [15, 17], [14, 16]]
...
'''
        ...
        ###########################################################################################
            ...
            if kpts_xyvd is not None:
                ...
                plane_points = find_plane_points(kpts_xyd)
                ...
                if object_coordinates is not None and plane_points is not None:
                    if check_time % 10 == 0:
                        coefficient = find_plane(plane_points)
                        create_dynamic_grid(vis, coefficient[0], coefficient[1], coefficient[2])

                    for i, a_b in enumerate(LINES_BODY):
                        a, b = a_b
                        if is_present(kpts_xyd[a - 1]) and is_present(kpts_xyd[b - 1]):
                            # 获取关键点的坐标
